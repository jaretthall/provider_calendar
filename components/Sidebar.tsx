...
const DraggableProviderItem: React.FC<DraggableProviderItemProps> = ({ provider, filters, onFilterChange, onEdit, onDelete, isAdmin }) => {
  ...
  return (
    <li 
      ref={setNodeRef} 
      style={style}
      className={`p-2 rounded-md group ${provider.isActive ? (isAdmin ? 'hover:bg-gray-700 cursor-grab' : 'hover:bg-gray-700') : 'opacity-60 hover:bg-gray-700'} ${isDragging ? 'shadow-lg' : ''}`}
    ...
    >
      <div className="flex items-center justify-between">
        <label className="flex items-center text-sm cursor-pointer w-full">
          <input
            type="checkbox"
            ...
            className="h-4 w-4 text-blue-500 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-offset-gray-800 mr-2 flex-shrink-0"
          />
          <span className={`w-3 h-3 rounded-full mr-2 flex-shrink-0 ${provider.color}`} aria-hidden="true"></span>
          <span id={`provider-name-${provider.id}`} className="truncate" title={provider.name}>{provider.name}</span>
          {!provider.isActive && <span className="ml-1 text-xs text-gray-400">(Inactive)</span>}
        </label>
        {isAdmin && (
          <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
            <button onClick={() => onEdit(provider)} className="p-1 text-gray-400 hover:text-white rounded-md focus:outline-none focus:ring-1 focus:ring-white" title="Edit Provider" aria-label={`Edit provider ${provider.name}`}>
              <EditIcon className="w-4 h-4" />
            </button>
            <button onClick={() => onDelete(provider)} className="p-1 text-red-400 hover:text-red-300 rounded-md focus:outline-none focus:ring-1 focus:ring-white" title="Delete Provider" aria-label={`Delete provider ${provider.name}`}>
        ...
        <nav className="flex-grow overflow-y-auto pr-1" aria-label="Main navigation filters">
          ...
          <div className="px-4 mb-4">
              <button 
                ...
                className="flex items-center justify-between w-full text-lg font-semibold text-gray-100 hover:text-white mb-2 p-1 rounded-md focus:outline-none focus:ring-1 focus:ring-white"
                aria-expanded={optionsOpen}
                aria-controls={optionsSectionId}
              >
                View Options
                <ChevronRightIcon className={`w-5 h-5 transform transition-transform ${optionsOpen ? 'rotate-90' : ''}`} />
              </button>
              {optionsOpen && (
                <div id={optionsSectionId} className="space-y-2 pl-2">
                   <div className="flex items-center">
                    ...
                    />
                    <label htmlFor="showVacationsFilter" className="ml-2 text-sm text-gray-300">Show Vacations / Time-off</label>
                  </div>
                </div>
              )}
          </div>
          ...
          <div className="px-4 mb-4">
            <div className="flex items-center justify-between">
              <button 
                ...
                className="flex items-center text-lg font-semibold text-gray-100 hover:text-white mb-2 p-1 rounded-md focus:outline-none focus:ring-1 focus:ring-white w-full text-left"
                aria-expanded={providersOpen}
                aria-controls={providersSectionId}
              >
                <UsersIcon className="w-5 h-5 mr-2 flex-shrink-0" /> Providers
                <ChevronRightIcon className={`w-5 h-5 ml-auto transform transition-transform flex-shrink-0 ${providersOpen ? 'rotate-90' : ''}`} />
              </button>
              {isAdmin && (
                <button
                  onClick={() => openModal('PROVIDER_FORM')}
                  className="p-1 text-blue-400 hover:text-blue-300 rounded-md focus:outline-none focus:ring-1 focus:ring-white flex-shrink-0 ml-2"
                  title="Add Provider"
            ...
            {providersOpen && (
              <ul id={providersSectionId} className="space-y-1 mt-1 max-h-60 overflow-y-auto pr-1">
                ...
                {providers.length === 0 && <li className="text-sm text-gray-400 italic pl-2">No providers yet.</li>}
              </ul>
            )}
          </div>
          ...
          <div className="px-4 mb-4">
            <div className="flex items-center justify-between">
              ...
              >
                <UsersGroupIcon className="w-5 h-5 mr-2 flex-shrink-0" /> Medical Assistants
                <ChevronRightIcon className={`w-5 h-5 ml-auto transform transition-transform flex-shrink-0 ${medicalAssistantsOpen ? 'rotate-90' : ''}`} />
              </button>
              {isAdmin && (
                <button
            ...
            {medicalAssistantsOpen && (
              <ul id={medicalAssistantsSectionId} className="space-y-1 mt-1 max-h-60 overflow-y-auto pr-1">
                {medicalAssistants.map(ma => (
                  <li key={ma.id} className={`p-2 rounded-md group ${ma.isActive ? 'hover:bg-gray-700' : 'opacity-60 hover:bg-gray-700'}`}>
                    <div className="flex items-center justify-between">
                      <label className="flex items-center text-sm cursor-pointer w-full">
                        <input
                          type="checkbox"
                          ...
                          className="h-4 w-4 text-blue-500 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-offset-gray-800 mr-2 flex-shrink-0"
                        />
                        <span className={`w-3 h-3 rounded-full mr-2 flex-shrink-0 ${ma.color}`} aria-hidden="true"></span>
                        <span id={`ma-name-${ma.id}`} className="truncate" title={ma.name}>{ma.name}</span>
                        {!ma.isActive && <span className="ml-1 text-xs text-gray-400">(Inactive)</span>}
                      </label>
                      {isAdmin && (
                        <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                          <button onClick={() => openModal('MEDICAL_ASSISTANT_FORM', { medicalAssistant: ma })} className="p-1 text-gray-400 hover:text-white rounded-md focus:outline-none focus:ring-1 focus:ring-white" title="Edit MA" aria-label={`Edit Medical Assistant ${ma.name}`}>
                            <EditIcon className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleDeleteMedicalAssistantFlow(ma)} className="p-1 text-red-400 hover:text-red-300 rounded-md focus:outline-none focus:ring-1 focus:ring-white" title="Delete MA" aria-label={`Delete Medical Assistant ${ma.name}`}>
                            <TrashIcon className="w-4 h-4" />
                          </button>
                        </div>
                      )}
                ...
                {medicalAssistants.length === 0 && <li className="text-sm text-gray-400 italic pl-2">No MAs yet.</li>}
              </ul>
            )}
          </div>
          ...
          <div className="px-4">
            <div className="flex items-center justify-between">
              ...
              >
                 <BriefcaseIcon className="w-5 h-5 mr-2 flex-shrink-0" /> Clinic Types
                 <ChevronRightIcon className={`w-5 h-5 ml-auto transform transition-transform flex-shrink-0 ${clinicsOpen ? 'rotate-90' : ''}`} />
              </button>
              {isAdmin && (
                <button
            ...
            {clinicsOpen && (
              <ul id={clinicsSectionId} className="space-y-1 mt-1 max-h-60 overflow-y-auto pr-1">
                {clinics.map(clinic => (
                  <li key={clinic.id} className={`p-2 rounded-md group ${clinic.isActive ? 'hover:bg-gray-700' : 'opacity-60 hover:bg-gray-700'}`}>
                    <div className="flex items-center justify-between">
                      <label className="flex items-center text-sm cursor-pointer w-full">
                        <input
                          type="checkbox"
                          ...
                          className="h-4 w-4 text-blue-500 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-offset-gray-800 mr-2 flex-shrink-0"
                        />
                        <span className={`w-3 h-3 rounded-full mr-2 flex-shrink-0 ${clinic.color}`} aria-hidden="true"></span>
                        <span id={`clinic-name-${clinic.id}`} className="truncate" title={clinic.name}>{clinic.name}</span>
                        {!clinic.isActive && <span className="ml-1 text-xs text-gray-400">(Inactive)</span>}
                      </label>
                       {isAdmin && (
                        <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                          <button onClick={() => openModal('CLINIC_TYPE_FORM', { clinicType: clinic })} className="p-1 text-gray-400 hover:text-white rounded-md focus:outline-none focus:ring-1 focus:ring-white" title="Edit Clinic Type" aria-label={`Edit clinic type ${clinic.name}`}>
                            <EditIcon className="w-4 h-4" />
                          </button>
                           <button onClick={() => handleDeleteClinicTypeFlow(clinic)} className="p-1 text-red-400 hover:text-red-300 rounded-md focus:outline-none focus:ring-1 focus:ring-white" title="Delete Clinic Type" aria-label={`Delete clinic type ${clinic.name}`}>
                            <TrashIcon className="w-4 h-4" />
                          </button>
                        </div>
                      )}
                 ...
                 {clinics.length === 0 && <li className="text-sm text-gray-400 italic pl-2">No clinic types yet.</li>}
              </ul>
            )}
          </div>
        </nav>
